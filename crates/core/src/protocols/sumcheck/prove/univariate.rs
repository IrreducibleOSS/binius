	...
	NTT: AdditiveNTT<P::Scalar>,
{
	...
	let coset = 0;
	ntt.inverse_transform(evals, coset, log_stride_batch, log_batch, skip_rounds)?;
		...
		izip!(1u32.., extrapolated_evals.chunks_exact_mut(evals.len()))
	{
		...
		ntt.forward_transform(extrapolated_chunk, coset, log_stride_batch, log_batch, skip_rounds)?;
	}
