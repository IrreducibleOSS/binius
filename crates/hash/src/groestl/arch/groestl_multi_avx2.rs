...
fn set_substates_par(substate_vals: [&[u8]; 4]) -> State {
    ...
    for i in 0..4 {
        new_state[2 * i] = unsafe{_mm256_loadu_si256(substate_vals[i][0..32].as_ptr() as *const __m256i)};
        new_state[2 * i + 1] = unsafe{_mm256_loadu_si256(substate_vals[i][32..64].as_ptr() as *const __m256i)};
    }
    ...
    // swaps because the SIMD instructions operate on the 128-bit lanes as opposed to the whole 256-bit value
...
fn get_substates_par_better(mut state: State) -> [[u8; 64]; 4] {
    ...
    let unbyteslice_permuatation_m256: __m256i =
        unsafe { std::mem::transmute(UNBYTESLICE_PERMUTATION_ARRAY) };
    ...
    for i in 0..8 {
        state[i] = unsafe { _mm256_permutexvar_epi8(unbyteslice_permuatation_m256, state[i]) };
    }
    ...
    for i in 0..4{
        unsafe {
            ...
            _mm256_storeu_si256(new_substates[i][32..64].as_mut_ptr() as *mut __m256i, state[2*i + 1]);
        }
    }
