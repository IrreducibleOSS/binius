...
fn get_substates_par_better(mut state: State) -> [[u8; 64]; 4] {
    ...
    for i in 0..4 {
        unsafe {
            _mm256_storeu_si256(
                new_substates[i][0..32].as_mut_ptr() as *mut __m256i,
                state[2 * i],
            );
            _mm256_storeu_si256(
        ...
        state[i] = unsafe {
            _mm256_xor_si256(
                _mm256_gf2p8mul_epi8(
                    gf2p8_2,
                    _mm256_xor_si256(
                        _mm256_gf2p8mul_epi8(gf2p8_2, y[(i + 3) % 8]),
                        z[(i + 7) % 8],
                    ),
                ),
                z[(i + 4) % 8],
...
mod tests {
    ...
    use crate::groestl::digest::digest::consts::U32;
    use crate::multi_digest::MultiDigest;
