...
impl ToTokens for ArithExprItem {
	fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
		let Self(expr) = self;
		tokens.extend(quote!(binius_math::ArithExpr::from(#expr)));
	}
}
	...
	match expr {
		syn::Expr::Path(path) => {
			...
			if let Some(i) = var_index {
				*expr = parse_quote!(binius_math::ArithExprNode::<#field>::Var(#i));
			} else {
		...
		syn::Expr::Lit(exprlit) => {
			if let syn::Lit::Int(int) = &exprlit.lit {
				...
				*expr = parse_quote!(binius_math::ArithExprNode::<#field>::Const(#value));
			}
		}
