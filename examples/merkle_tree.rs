...
use crate::{
	arithmetization::MerkleTreeCS,
	model::{MerklePath, MerkleTree, MerkleTreeTrace},
};
...
pub mod arithmetization {
	...
	/// - `MerkleTreeCS`: The constraint system for the Merkle tree, including all tables and
	///   channels.
	/// - `NodesTable` and `RootTable`: Table definitions for Merkle path steps and root
	///   checks.
	...
	use crate::model::{
		MerklePath, MerklePathEvent, MerkleRootEvent, MerkleTree, MerkleTreeTrace, NodeFlushToken,
		RootFlushToken,
	};
	...
	impl NodesTable {
		pub fn new(
			...
			let left_columns: [Col<BinaryField8b, 4>; 8] =
				table.add_committed_multiple("left_columns");
			...
			let right_packed: [Col<B32>; 8] = array::from_fn(|i| {
				table.add_packed(format!("right_packed[{i}]"), right_columns[i])
			});
...
fn main() -> Result<()> {
	...
	let paths = (0..100)
		.map(|i| {
			let index = rng.gen_range(0..1 << 20);
			MerklePath {
				root_id: 0,
				index,
				leaf: leaves[index],
